...
```sh
./target/release/omniblock-node -h
```
...
```bash
./target/release/omniblock-node --dev
```

Purge the development chain's state:

```bash
./target/release/omniblock-node purge-chain --dev
```

Start the development chain with detailed logging:

```bash
RUST_BACKTRACE=1 ./target/release/omniblock-node -ldebug --dev
```
...
$ ./target/release/omniblock-node --dev --base-path ./my-chain-state/
...
```shell
./target/release/omniblock-node --help
```
...
(`cargo build --release && ./target/release/omniblock-node --dev --ws-external`)
by appending your own. A few useful ones are as follow.

```bash
# Run Substrate node without re-compiling
./scripts/docker_run.sh ./target/release/omniblock-node --dev --ws-external

# Purge the local dev chain
./scripts/docker_run.sh ./target/release/omniblock-node purge-chain --dev
